...
#[test]
fn test_deserialize_empty_negative() {
    assert!(serde_json::from_str::<NonEmptyAndNonNegative>("[\"\", -1]").is_err());
}

#[test]
fn test_deserialize_empty_not_negative() {
    assert!(serde_json::from_str::<NonEmptyAndNonNegative>("[\"\", 1]").is_err());
}

#[test]
fn test_deserialize_not_empty_negative() {
    assert!(serde_json::from_str::<NonEmptyAndNonNegative>("[\"Lucas\", -1]").is_err());
}

#[test]
fn test_deserialize_not_empty_not_negative() {
